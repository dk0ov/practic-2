...
public static class DatabaseRequests
{
    ...
    public static void GetCarsQuery()
    {
        ...
        while (reader.Read())
        {
            Console.WriteLine($"Id типа машины: {reader[1]} Модель: {reader[2]} Номер: {reader[3]} Количество пассажиров: {reader[4]}");
        }
    }

    public static void AddCarsQuery(int car_type, string name, string car_number, int quantity)
    {
        var querySql = $"INSERT INTO car(id_type_car, name, state_number, number_passengers) VALUES ('{car_type}', '{name}', '{car_number}', '{quantity}')";
        using var cmd = new NpgsqlCommand(querySql, DatabaseService.GetSqlConnection());
        cmd.ExecuteNonQuery();
    }
    ...
    public static void GetRouteQuery()
    {
        ...
        while (reader.Read())
        {
            Console.WriteLine($"Id водителя: {reader[1]} Id машины: {reader[2]} Id маршрута: {reader[3]} Количество пассажиров: {reader[4]}");
        }
    }

    public static void AddRouteQuery(int driver, int car, int itinerary, int quantity)
    {
        var querySql = $"INSERT INTO route(id_driver, id_car, id_itinerary, number_passengers) VALUES ('{driver}', '{car}', '{itinerary}', '{quantity}')";
        using var cmd = new NpgsqlCommand(querySql, DatabaseService.GetSqlConnection());
        cmd.ExecuteNonQuery();
    }
}